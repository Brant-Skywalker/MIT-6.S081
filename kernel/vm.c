...
kvmmake(void)
{
  ...
  kpgtbl = (pagetable_t) kalloc();  // First allocates a page of physical memory to hold the root page.
  ...
  kvmmap(kpgtbl, KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X); // Kernel instructions.
  ...
  // map kernel stacks
  proc_mapstacks(kpgtbl);  // This function allocates a kernel stack for each process.
...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    if((pte = walk(pagetable, a, 0)) == 0)  // Find PTEs.
      panic("uvmunmap: walk");
    if((*pte & PTE_V) == 0)
    ...
    if(do_free){
      ...
      kfree((void*)pa);  // Free the physical memory.
    }
    *pte = 0;
  }
}
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  ...
  for(a = oldsz; a < newsz; a += PGSIZE){
    mem = kalloc();  // Allocates physical memory.
    if(mem == 0){
      uvmdealloc(pagetable, a, oldsz);
      return 0;
    }
    memset(mem, 0, PGSIZE);
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_W|PTE_X|PTE_R|PTE_U) != 0){  // Adds PTEs to the user page table.
      kfree(mem);
      uvmdealloc(pagetable, a, oldsz);
      return 0;
    }
